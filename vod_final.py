...
""")
    print(f"{LIGHT_WHITE}You had {inventory} in your possesion.{LIGHT_CYAN}\n\n")
    quit()
...
def secret_question():
    ...
    for jobs in occupation:
        print(jobs)
    answer = input()
    if len(answer) > 1 or not answer.isdigit():  secret_question()
    answer =int(answer)
    if answer== 1:
...
def id_form():
    ...
    def age_check():
        ...
        age = input("")
        if len(age) > 3 or not age[0].isdigit():  age_check()
        age = int(age)
        try: 
...
def ticket_check():
    ...
    for check in ticket_list:
        print(check)
    answer = input()
    if len(answer) > 1 or not answer.isdigit():  ticket_check()
    answer =int(answer)
    if answer == 1:
        stress_level += 1
...
def open_door_pos():
    ...
    for pick in react:
        print(pick)
    answer = input()
    if len(answer) > 1 or not answer.isdigit():  open_door_pos()
    answer =int(answer)
    if answer == 1:
        global life_jacket
        if life_jacket ==1:
...
def open_door_fire():
    ...
    for pick in react:
        print(pick)
    answer = input()
    if len(answer) > 1 or not answer.isdigit():  open_door_fire()
    answer =int(answer)
    if answer == 1:
    ...
    *✲*´*。.❄¯*✲。❄。*¨`*✲´*。❄¨`*
    ...
    def pick_react():
        ...
        for pick in react:
            print(pick)
        answer = input()
        if len(answer) > 1 or not answer.isdigit():  posidon()
        answer =int(answer)
        if answer== 1:
            global life_jacket 
            life_jacket +=1
    ...
    *✲*´*。.❄¯*✲。❄。*¨`*✲´*。❄¨`*
    ...
    def pick_react():
        ...
        for pick in react:
            print(pick)
        answer = input()
        if len(answer) > 1 or not answer.isdigit():  fire_ship()
        answer =int(answer)
        if answer== 1:
            global life_jacket
            life_jacket +=1
